...
for i in range(3):
    for j in range(2):
        hTau1P0N = root_file_tau.Get('1p0n_tau_' + names[j] + '_eff_' + regions[i])
        hPion1P0N = root_file_pi.Get('1p0n_pi_' + names[j] + '_eff_' + regions[i])
        hTau3P0N = root_file_tau.Get('3p0n_tau_' + names[j] + '_eff_' + regions[i])
        hPion3P0N = root_file_pi.Get('3p0n_pi_' + names[j] + '_eff_' + regions[i])

        # Compare pion 1P0N and pion 3P0N bin by bin
        print(f'Comparing 1P0N and 3P0N pion {names[j]} in {regions[i]}')

        # Loop over number of bins (10)
        for k in range(1, 11):
            print(f'Bin {k}: 1P0N: {hPion1P0N.GetBinContent(k)} +/- {hPion1P0N.GetBinError(k)}, 1P0N Cubed: {(hPion1P0N.GetBinContent(k))**3} +/- {3*(hPion1P0N.GetBinContent(k)**2)*(hPion1P0N.GetBinError(k))}, 3P0N: {hPion3P0N.GetBinContent(k)} +/- {hPion3P0N.GetBinError(k)}')
