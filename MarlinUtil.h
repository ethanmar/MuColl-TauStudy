...
/** Function to get the accumulated sum of the energy per event and the number
 * of particles within different categories at IP. The return values are given
 * in the array accumulatedEnergies of size 21 with the following content. Only
 * MC particles with generator status 1 are considered:
 *
 *  accumulatedEnergies[0]  : energy of MC particles which is possible to
 * measure in calorimeter (This means muons are presented as 1.6 GeV each)
 ...
 *  accumulatedEnergies[10] : number of MC particles which are possible to
 * measure accumulatedEnergies[11] : number of MC particles lost in tube
 ...
 *  accumulatedEnergies[20] : energy of MC particles which is possible to
 * measure (real sum [see 0])
 */
...
void printRecoParticle(lcio::ReconstructedParticle *recoParticle,
                       double bField = 4.0);
int countAllSimTrackerHits(lcio::LCEvent *evt, lcio::MCParticle *MCP);
int countAllSimCalorimeterHits(lcio::LCEvent *evt, lcio::MCParticle *MCP,
                               double &accumulatedSimCaloEnergy);
double getEnergyDepositedInFullCalorimeter(lcio::LCEvent *evt);

/** Return track weight contribution encoded as trackwgt =
 * (int(wgt)%10000)/1000. by the encodeTrackAndClusterWeights function for the
...
/** Return cluster weight contribution encoded as clusterwgt =
 * (int(wgt)/10000)/1000. by the encodeTrackAndClusterWeights function for the
 * ReconstructedParticle-MCParticle (or vise-versa) type of relations.
 */
...
/** Return a single number that encodes in itself two numbers representing track
 * and cluster weights given as arguments. For tracks and clusters, the weight
 * is the sum of hits from the considered true particle divided by the sum of
 * all hits. It is used inside RecoMCTruthLinker.
 */
inline float encodeTrackAndClusterWeights(const float trackWeight,
                                          const float clusterWeight) {
  return int(clusterWeight * 1000) * 10000 + int(trackWeight * 1000);
}

} // namespace MarlinUtil

// FIXME: the whole MC particle related stuff should be placed somewhere else
// (see MarlinUtil as well)
