#include "MyTauFinder/MyEvaluateTauTauMuMu.h"
...
MyEvaluateTauTauMuMu aEvaluateTauTauMuMu;

MyEvaluateTauTauMuMu::MyEvaluateTauTauMuMu()
    : Processor("MyEvaluateTauTauMuMu") {
  // modify processor description
  _description = "MyEvaluateTauTauMuMu checks performance of TauFinder and "
...
void MyEvaluateTauTauMuMu::init() {
  ...
  tree->Branch("mcMuEta", mcMuEta, "mcMuEta[2]/F");
  tree->Branch("mcMuForward", &mcMuForward, "mcMuForward/I");
}

void MyEvaluateTauTauMuMu::processRunHeader(LCRunHeader *) { _nRun++; }

void MyEvaluateTauTauMuMu::processEvent(LCEvent *evt) {
  ...
  int count_mu = 0;
  MCParticle *particleMC = NULL;

  for (int k = 0; k < colMC->getNumberOfElements(); k++) {
    particleMC = static_cast<MCParticle *>(colMC->getElementAt(k));
    if (particleMC) {
      if (abs(particleMC->getPDG()) == 13 &&
          particleMC->getGeneratorStatus() == 1) {
        const double *mom_mu = particleMC->getMomentumAtEndpoint();
        const double eta_mu =
            fabs(TMath::ATanH(mom_mu[2] / TMath::Sqrt(mom_mu[0] * mom_mu[0] +
                                                 mom_mu[1] * mom_mu[1] +
                                                 mom_mu[2] * mom_mu[2])));
        mcMuEta[count_mu] = eta_mu;
...
void MyEvaluateTauTauMuMu::LoopDaughters(MCParticle *particle, double &Evis,
...
void MyEvaluateTauTauMuMu::LoopDaughtersRelation(
...
int MyEvaluateTauTauMuMu::FindDecayMode(MCParticle *tau) {
  /*
   *Decay modes for tau-:
...
void MyEvaluateTauTauMuMu::check(LCEvent *) {
  // nothing to check here - could be used to fill checkplots in reconstruction
  // processor
}

void MyEvaluateTauTauMuMu::end() {

  streamlog_out(DEBUG) << "MyEvaluateTauTauMuMu::end()  " << name()
