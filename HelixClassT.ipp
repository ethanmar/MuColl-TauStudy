...
void HelixClassT<FloatT>::Initialize_VP(FloatT *pos, FloatT *mom, FloatT q,
                                        FloatT B/*, FloatT *sigma_pos,
                                        FloatT *sigma_mom*/) {
  ...
  double xCentre =
      double(pos[0]) + radius * double(cos(_phiMomRefPoint - _const_pi2 * q));
  double yCentre =
      double(pos[1]) + radius * double(sin(_phiMomRefPoint - _const_pi2 * q));

  double d0;

  if (q > 0) {
    d0 = double(q) * radius -
         double(sqrt(xCentre * xCentre + yCentre * yCentre));
  } else {
    d0 = double(q) * radius +
         double(sqrt(xCentre * xCentre + yCentre * yCentre));
  }
  ...
  _d0 = FloatT(d0);
  /*
  double k = 1. / double(sqrt(xCentre * xCentre + yCentre * yCentre));
  double derd0_r = fabs(
      double(q) +
      k * (radius +
           double(pos[0]) * double(cos(_phiMomRefPoint - _const_pi2 * q)) +
           double(pos[1]) * double(sin(_phiMomRefPoint - _const_pi2 * q))));
  double derd0_x0 =
      fabs(k * (radius * double(cos(_phiMomRefPoint - _const_pi2 * q)) +
                double(pos[0])));
  double derd0_y0 =
      fabs(k * (radius * double(sin(_phiMomRefPoint - _const_pi2 * q)) +
                double(pos[1])));
  double derd0_phimom =
      fabs(k * (-radius * double(pos[0]) *
                    double(sin(_phiMomRefPoint - _const_pi2 * q)) +
                radius * double(pos[1]) *
                    double(cos(_phiMomRefPoint - _const_pi2 * q))));
  double sigma2_r =
      fabs(mom[0] / _pxy) * fabs(mom[0] / _pxy) * sigma_mom[0] * sigma_mom[0] +
      fabs(mom[1] / _pxy) * fabs(mom[1] / _pxy) * sigma_mom[1] * sigma_mom[1];
  double sigma2_x0 = sigma_pos[0] * sigma_pos[0];
  double sigma2_y0 = sigma_pos[1] * sigma_pos[1];
  double sigma2_phimom =
      (sigma_mom[0] * sigma_mom[0] + sigma_mom[1] * sigma_mom[1]) /
      (_pxy * _pxy);

  double sigmad0 = double(sqrt(derd0_r * derd0_r * sigma2_r +
                               derd0_x0 * derd0_x0 * sigma2_x0 +
                               derd0_y0 * derd0_y0 * sigma2_y0 +
                               derd0_phimom * derd0_phimom * sigma2_phimom));
  _sigmad0 = FloatT(sigmad0);
  */
  ...
  //       std::cout << " RC = " <<
  //       sqrt(_xCentre*_xCentre+_yCentre*_yCentre) << std::endl;
...
template <typename FloatT>
void HelixClassT<FloatT>::Initialize_Canonical(FloatT phi0, FloatT d0,
                                               FloatT z0, FloatT omega,
...
template <typename FloatT>
void HelixClassT<FloatT>::Initialize_BZ(FloatT xCentre, FloatT yCentre,
                                        FloatT radius, FloatT bZ, FloatT phi0,
                                        FloatT B, FloatT signPz,
                                        FloatT zBegin) {
  ...
  _phiRefPoint =
      atan2(_referencePoint[1] - _yCentre, _referencePoint[0] - _xCentre);
...
template <typename FloatT> FloatT HelixClassT<FloatT>::getPhi0() const {
  if (_phi0 < 0.0)
...
template <typename FloatT>
FloatT HelixClassT<FloatT>::getPointInXY(FloatT x0, FloatT y0, FloatT ax,
                                         FloatT ay, FloatT *ref,
                                         FloatT *point) const {
...
template <typename FloatT>
FloatT HelixClassT<FloatT>::getPointOnCircle(FloatT Radius, FloatT *ref,
                                             FloatT *point) const {
  ...
  FloatT phiStar =
      Radius * Radius + distCenterToIP * distCenterToIP - _radius * _radius;
...
template <typename FloatT>
FloatT HelixClassT<FloatT>::getPointInZ(FloatT zLine, FloatT *ref,
                                        FloatT *point) const {
...
template <typename FloatT>
FloatT HelixClassT<FloatT>::getDistanceToPoint(FloatT const *xPoint,
  ...
  FloatT phi0 =
      atan2(_referencePoint[1] - _yCentre, _referencePoint[0] - _xCentre);
  // calculate distance to XYprojected centre of Helix, comparing this with
  // distance to radius around centre gives DistXY
  FloatT DistXY = (_xCentre - xPoint[0]) * (_xCentre - xPoint[0]) +
                  (_yCentre - xPoint[1]) * (_yCentre - xPoint[1]);
  DistXY = sqrt(DistXY);
  ...
  if (fabs(_tanLambda * _radius) > 1.0e-20) {
    FloatT xCircles =
        phi0 - phi -
        _charge * (xPoint[2] - _referencePoint[2]) / (_tanLambda * _radius);
    xCircles = xCircles / _const_2pi;
    int n1, n2;
...
template <typename FloatT>
FloatT
HelixClassT<FloatT>::getDistanceToPoint(const std::vector<FloatT> &xPoint,
                                        FloatT distCut) const {
  // calculate distance to XYprojected centre of Helix, comparing this with
  // distance to radius around centre gives DistXY
  ...
  // If this is bigger than distCut, we dont have to know how much bigger this
  // is
  if (DistXY > distCut) {
    return DistXY;
  }
  ...
  FloatT DistZ = -tempz - _charge * tanradius *
                              (_const_2pi * ((FloatT)nCircles) - phidiff);
  return sqrt(DistXY * DistXY + DistZ * DistZ);
} // getDistanceToPoint(vector,FloatT)

template <typename FloatT>
FloatT HelixClassT<FloatT>::getDistanceToPoint(const FloatT *xPoint,
                                               FloatT distCut) const {
  std::vector<FloatT> xPosition(
      xPoint, xPoint + 3); // We are expecting three coordinates, must be +3,
...
template <typename FloatT>
FloatT HelixClassT<FloatT>::getDistanceToHelix(HelixClassT *helix, FloatT *pos,
                                               FloatT *mom) const {
  ...
  if (rad1 + rad2 < distance) {
    ...
    singlePoint = false;
    FloatT cosAlpha = 0.5 * (distance * distance + rad2 * rad2 - rad1 * rad1) /
                      (distance * rad2);
  ...
  if (singlePoint) {
    ...
    //     std::cout << "(xSect1,ySect1)=(" << xSect1 << "," << ySect1 << ")" <<
    //     std::endl; std::cout << "(xSect2,ySect2)=(" << xSect2 << "," <<
    //     ySect2 << ")" << std::endl;
...
template <typename FloatT>
void HelixClassT<FloatT>::getExtrapolatedMomentum(FloatT *pos,
                                                  FloatT *momentum) const {
