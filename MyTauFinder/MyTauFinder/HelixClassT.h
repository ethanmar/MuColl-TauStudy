 ...
 *				   float bZ, float phi0, float B, float
 *signPz,<br> float zBegin):<br> initialization of helix is done according to
 *the following<br> parameterization<br> x = xCentre + radius*cos(bZ*z +
 *phi0)<br> y = yCentre + radius*sin(bZ*z + phi0)<br> where (x,y,z) - position
 *of point on the helix<br>
 ...
 *    3) void Initialize_Canonical(float phi0, float d0, float z0, float
 *omega,<br> float tanlambda, float B) :<br> canonical (LEP-wise)
 *parameterisation with the following parameters<br>
 ...
 *       - z0 - z coordinate of the point of closest approach in R-Phi
 *plane;<br>
...
template <typename FloatT> class HelixClassT {
public:
  /**
  ...
  void Initialize_VP(FloatT *pos, FloatT *mom, FloatT q, FloatT B/*,
                     FloatT *sigma_pos, FloatT *sigma_mom*/);
   ...
   *     - zBegin is the z coordinate of the reference point<br>
   */
  void Initialize_BZ(FloatT xCentre, FloatT yCentre, FloatT radius, FloatT bZ,
                     FloatT phi0, FloatT B, FloatT signPz, FloatT zBegin);
  /**
  ...
  //FloatT getSigmaD0() const { return _sigmad0; }
   ...
   */
  FloatT getPointInXY(FloatT x0, FloatT y0, FloatT ax, FloatT ay, FloatT *ref,
                      FloatT *point) const;
   ...
   * <br> If the R-Phi distance is not too big, than the exact 3D distance is
   * returned <br> This function can be used, if the exact distance is not
   * always needed <br>
   */
  FloatT getDistanceToPoint(const FloatT *xPoint, FloatT distCut) const;
  FloatT getDistanceToPoint(const std::vector<FloatT> &xPoint,
...
private:
  ...
  FloatT _sigmad0 = 0.0;     // sigma d0 in canonical parameterisation
