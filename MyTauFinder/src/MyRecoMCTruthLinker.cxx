#include "MyTauFinder/MyRecoMCTruthLinker.h"
...
MyRecoMCTruthLinker aRecoMCTruthLinker;
...
MyRecoMCTruthLinker::MyRecoMCTruthLinker() : Processor("MyRecoMCTruthLinker") {

  // modify processor description
  _description = "links RecontructedParticles to the MCParticle based on "
                 "number of hits used";
  ...
  registerInputCollection(LCIO::TRACK, "TrackCollection",
                          ...
                          _trackCollectionName, std::string("MarlinTrkTracks"));

  registerInputCollection(LCIO::CLUSTER, "ClusterCollection",
  ...
  registerProcessorParameter(
      ...
      _use_tracker_hit_relations, bool(true));
  ...
  registerInputCollections("LCRelation", "TrackerHitsRelInputCollections",
                           "Name of the lcrelation collections, that link the "
                           "TrackerHits to their SimTrackerHits.",
  ...
  caloHitsInputColNamesDefault.push_back(
  ...
  registerInputCollections(LCIO::SIMCALORIMETERHIT, "SimCaloHitCollections",
  ...
  registerInputCollections("LCRelation", "SimCalorimeterHitRelationNames",
                           "Name of the  lcrelation collections, that link the "
                           "SimCalorimeterHit to CalorimeterHit",
  ...
  registerOutputCollection(LCIO::LCRELATION, "MCTruthTrackLinkName",
                           "Name of the trackMCTruthLink output collection",
                           _mCTruthTrackLinkName, std::string(""));

  registerOutputCollection(
      LCIO::LCRELATION, "TrackMCTruthLinkName",
      "Name of the trackMCTruthLink output collection - not created if empty()",
      _trackMCTruthLinkName, std::string(""));

  registerOutputCollection(LCIO::LCRELATION, "ClusterMCTruthLinkName",
                           "Name of the clusterMCTruthLink output collection - "
                           "not created if empty()",
                           _clusterMCTruthLinkName, std::string(""));

  registerOutputCollection(LCIO::LCRELATION, "MCTruthClusterLinkName",
                           "Name of the MCTruthClusterLink output collection",
                           _mCTruthClusterLinkName, std::string(""));

  registerProcessorParameter(
      "FullRecoRelation",
      "true: All reco <-> true relations are given, with weight = 10000*calo "
      "weight+"
      "track weight (weights in permill). false: Only highest contributor "
      "linked,"
      "and only to tracks, not clusters if there are any tracks",
      _FullRecoRelation, bool(false));

  registerOutputCollection(
      LCIO::LCRELATION, "RecoMCTruthLinkName",
      "Name of the RecoMCTruthLink output collection - not created if empty()",
      _recoMCTruthLinkName, std::string("RecoMCTruthLink"));

  registerOutputCollection(LCIO::LCRELATION, "MCTruthRecoLinkName",
                           "Name of the MCTruthRecoLink output collection",
                           _mCTruthRecoLinkName, std::string(""));

  registerOutputCollection(LCIO::LCRELATION, "CalohitMCTruthLinkName",
                           "Name of the updated calo-hit MCTruthLink output "
                           "collection - not created if empty()",
                           _calohitMCTruthLinkName, std::string(""));
  ...
  registerOutputCollection(LCIO::MCPARTICLE, "MCParticlesSkimmedName",
                           "Name of the skimmed MCParticle  output collection "
                           "- not created if empty()",
                           _mcParticlesSkimmedName, std::string(""));
  // <<<<<<<
  ...
  registerProcessorParameter(
      "daughtersECutMeV",
      "energy cut for daughters that are kept from KeepDaughtersPDG", _eCutMeV,
      float(10.));

  registerProcessorParameter("SaveBremsstrahlungPhotons",
                             "save photons from Brems",
                             _saveBremsstrahlungPhotons, bool(false));

  registerProcessorParameter("UsingParticleGun",
                             "If Using Particle Gun Ignore Gen Stat",
                             _using_particle_gun, bool(false));

  registerProcessorParameter("BremsstrahlungEnergyCut",
                             "energy cut for Brems that are kept",
                             _bremsstrahlungEnergyCut, float(1.));

  registerProcessorParameter(
      "InvertedNonDestructiveInteractionLogic",
      "Work-around Mokka bug in vertex-is-not-endpoint-of-parent flag (logic "
      "inverted)",
      _invertedNonDestructiveInteractionLogic, bool(false));
}

void MyRecoMCTruthLinker::init() {
  ...
  for (IntVec::iterator it = _pdgVec.begin(); it != _pdgVec.end(); ++it) {
    ...
    if (*it < 0) {
      ...
      streamlog_out(WARNING)
          << " init: negative PDG given - only abs value is used : " << *it
          << std::endl;
    }
  ...
  if (!_use_tracker_hit_relations) {
    ...
    streamlog_out(WARNING)
        << "  ====== UseTrackerHitRelations=false => not using "
           "TrackerHit-SimTrackerHit-Relations  but getRawHits() instead - \n"
        << "         this is probably not what you want (only for backward "
...
void MyRecoMCTruthLinker::processRunHeader(LCRunHeader * /*run*/) { _nRun++; }

void MyRecoMCTruthLinker::processEvent(LCEvent *evt) {

  _nEvt++;

  streamlog_out(MESSAGE0) << " processEvent " << evt->getEventNumber() << "  - "
  ...
  if (!haveTracks) {
    streamlog_out(DEBUG9) << " Track collection : " << _trackCollectionName
                          << " not found - cannot create relation "
                          << std::endl;
  ...
  // find cluster to MCParticle relations and the updated calohit to MCParticle
  // relations.
  ...
  try {
    clusterCol = evt->getCollection(_clusterCollectionName);
  } catch (DataNotAvailableException &) {
    haveClusters = false;
  }
  ...
  if (haveTracks && haveClusters) {
    streamlog_out(WARNING) << "CHECKPOINT0" << std::endl;

    clusterLinker(evt, mcpCol, clusterCol, &ctrlcol, &trclcol, &chittrlcol);

    streamlog_out(WARNING) << "CHECKPOINTFINAL" << std::endl;

    if (_OutputClusterTruthRelation)
      evt->addCollection(ctrlcol, _clusterMCTruthLinkName);
    if (_OutputTruthClusterRelation)
      evt->addCollection(trclcol, _mCTruthClusterLinkName);
    if (_OutputCalohitRelation)
      evt->addCollection(chittrlcol, _calohitMCTruthLinkName);
  }

  // combine track and cluster to MCParticle relations to the reconstructed
  // particle to MCParticle relation
  LCCollection *particleCol = 0;
  bool haveRecoParticles = true;

  try {
    particleCol = evt->getCollection(_recoParticleCollectionName);
  } catch (DataNotAvailableException &) {
    haveRecoParticles = false;
  }

  streamlog_out(WARNING) << "CHECKPOINT8" << std::endl;

  if (!haveRecoParticles) {
    streamlog_out(DEBUG9) << " ReconstructedParticle collection : "
                          << _recoParticleCollectionName
                          << " not found - cannot create relation "
                          << std::endl;
  }
  ...
  if (haveRecoParticles && haveTracks && haveClusters && haveCaloHitRel &&
    ...
    particleLinker(mcpCol, particleCol, ttrlcol, ctrlcol, trtlcol, trclcol,
                   &ptrlcol, &trplcol);

    evt->addCollection(ptrlcol, _recoMCTruthLinkName);
    if (_OutputTruthRecoRelation)
      evt->addCollection(trplcol, _mCTruthRecoLinkName);
  }

  if (haveTracks && haveClusters && haveCaloHitRel &&
      !_mcParticlesSkimmedName.empty()) {
  ...
  // If either collection has not been added to the event, we have to delete it
  // now! Don't delete them before, because they are used
  if (!_OutputClusterTruthRelation) {
    delete ctrlcol;
  }
  if (!_OutputTruthClusterRelation) {
  ...
  delete _navMergedTrackerHitRel;
  _navMergedTrackerHitRel = nullptr;
  delete _navMergedCaloHitRel;
  _navMergedCaloHitRel = nullptr;
  delete _mergedCaloHitRelCol;
  _mergedCaloHitRelCol = nullptr;
  delete _mergedTrackerHitRelCol;
  _mergedTrackerHitRelCol = nullptr;
}

void MyRecoMCTruthLinker::trackLinker(LCEvent *evt, LCCollection *mcpCol,
                                      LCCollection *trackCol,
                                      LCCollection **ttrlcol,
                                      LCCollection **trtlcol) {

  // merge all the SimTrackerHit - TrackerHit relations into on collection and
  // set up the combined navigator
  ...
  streamlog_out(DEBUG6)
      << " *** Sorting out Track<->MCParticle using simHit<->MCParticle."
      << std::endl;

  streamlog_out(DEBUG) << "Number of tracks: " << nTrack << std::endl;
  ...
  for (int i = 0; i < nTrack; ++i) {
    ...
    for (TrackerHitVec::const_iterator hitIt = trkHits.begin();
      ...
      const LCObjectVec &simHits = _use_tracker_hit_relations
                                       ? *(this->getSimHits(hit))
                                       : hit->getRawHits();
      MCParticle *mcp2 = 0;
    ...
    if (nSimHit == 0) {
      ...
      streamlog_out(WARNING)
          << " No simulated tracker hits found. Set UseTrackerHitRelations to "
             "true in steering file to enable using TrackerHit relations if "
             "they are available."
          << std::endl;
      streamlog_out(WARNING) << trk->id() << " " << i << " "
      ...
      for (TrackerHitVec::const_iterator hitIt = trkHits.begin();
        ...
        TrackerHit *hit = *hitIt; // ... and a seen hit ...
        streamlog_out(WARNING)
            << hit->getPosition()[0] << " " << hit->getPosition()[1] << " "
    ...
    MCParticle *mother = 0;
    MCParticleVec
        theMCPs; // vector that will contain all true particles contributing
    theMCPs.reserve(1000);
    std::vector<int> MCPhits;
    MCPhits.reserve(1000);
    int ifound = 0;

    for (MCPMap::iterator it =
             mcpMap.begin(); // iterate trough the map, map->first is the
         it != mcpMap.end(); ++it) { // true particle, map->second is the number
                                     // of times this true particle got mapped,
                                     // ie. the number of hits it produced.

      mother = (it->first->getParents().size() != 0
                    ? dynamic_cast<MCParticle *>(it->first->getParents()[0])
                    : 0); // mother of the true particle.

      if (_using_particle_gun ||
          it->first->getGeneratorStatus() ==
              1) { // genstat 1 particle, ie. it is a bona fide
        ...
        if (mother != 0) { // if it has a parent, save it
          ...
          streamlog_out(WARNING) << " track has hit(s) from a non-generator "
                                    "particle with no parents ?!! "
                                 << std::endl;
        }
      }
    ...
    for (unsigned ihit = 0; ihit < trkHits.size(); ++ihit) {
      if (UTIL::BitSet32(
              trkHits[ihit]
                  ->getType())[UTIL::ILDTrkHitTypeBit::COMPOSITE_SPACEPOINT]) {
        nHit += 2;
      } else {
        nHit += 1;
      }
    ...
    for (int iii = 0; iii < ifound; iii++) {
      ...
      streamlog_out(DEBUG4)
          << "    track " << trk->id() << " has " << MCPhits[iii] << " hits of "
          << nSimHit << " SimHits (" << nHit << " TrackerHits) "
          << " weight = " << weight << " , "
          << " inv rel weight = " << inv_weight << "  "
          << " of MCPart with pdg : " << theMCPs[iii]->getPDG() << " , SimHits "
          << Total_SimHits_forMCP
          << " and genstat : " << theMCPs[iii]->getGeneratorStatus()
          << " id: " << theMCPs[iii]->id() << std::endl;
    }
  ...
  streamlog_out(DEBUG6) << " *** Sorting out Track<->MCParticle : DONE "
                        << std::endl;
  streamlog_out(DEBUG6) << " *** track linking complete, create collection "
                        << std::endl;
...
void MyRecoMCTruthLinker::clusterLinker(LCEvent *evt, LCCollection *mcpCol,
  ...
  streamlog_out(DEBUG6) << " *** Sorting out simHit<->MCParticle connections, "
                           "and find corresponding calo hits "
                        << std::endl;

  // This step is needed for cluster (contrary to the tracks above), because the
  // first few branchings in the shower is kept in the MCParticle collection. We
  // therefor need to back-track to the particle actually entering the
  // calorimeter. The originator of a hit sometimes IS a generator particle, in
  // which case there is no problem. Otherwise, the criteria to keep a particle
  // as a hit originator is:
  //  The particle is an ancestor of the one initial linked to the hit.
  //      AND
  //  [
  //   The particle is a generator particle
  //        OR
  //   The particle starts in the tracker, ends in calorimeter. That it starts
  //   in the tracker
  //    is determined by the fact that it's mother ends there. However, note the
  //    posibility of "non-destuctive interactions", where a particles mother
  //    does *not* end at the point where the particle is created !
  //        OR
  //   The particle, or one of it's ancestors is a back-scatterer. This case is
  //   further treated in the
  //    loop over clusters, to catch the rather common case that a
  //    back-scattered particle re-enters the calorimeter close to it's
  //    start-start point (think 15 MeV charged particle!) and the hits it
  //    produces is in the same cluster as that of the initiator of the shower
  //    the backscatter come from. (This can't be detected in this first loop,
  //    since we don't know about clusters here)
  //   ]
  //   A map is set up, so obviously the first check is wether the MCP of the
  //   hit has already been treated when looking at some previous hit, in which
  //   case one just thake that association.

  for (unsigned i = 0, iN = _simCaloHitCollectionNames.size(); i < iN; ++i) {
    const LCCollection *col = 0;
    try {
      col = evt->getCollection(_simCaloHitCollectionNames[i]);
    } catch (DataNotAvailableException &) {
    }
    if (col) {
      ...
      streamlog_out(DEBUG6) << " ================= " << std::endl;
      streamlog_out(DEBUG6)
          << " Treating sim-hits in " << _simCaloHitCollectionNames[i]
          << ". It has " << col->getNumberOfElements() << " hits " << std::endl;

      for (int j = 0, jN = col->getNumberOfElements(); j < jN; ++j) {

        SimCalorimeterHit *simHit = (SimCalorimeterHit *)col->getElementAt(j);
        LCObjectVec caloHits =
            _navMergedCaloHitRel->getRelatedFromObjects(simHit);
        if (caloHits.size() == 0) {
          continue;
        }
        if (caloHits.size() != 1) {
        ...
        // for split hits, need to get calib factor over sum of all rec hits
        // (DJ)
        double allRecoEn(0);
        ...
        streamlog_out(DEBUG4) << "  ================= " << std::endl;
        streamlog_out(DEBUG4)
            << "    Treating hit " << j << " sim hit id " << simHit->id()
            << " calo hit id " << caloHit->id() << " nb contributions "
            << simHit->getNMCContributions() << std::endl;
        streamlog_out(DEBUG3)
            << "      ncalo hits : " << caloHits.size() << " calib factor "
            << calib_factor << " position " << simHit->getPosition()[0] << " "
            << simHit->getPosition()[1] << " " << simHit->getPosition()[2]
            << " " << std::endl;
        ...
        for (int k = 0; k < simHit->getNMCContributions(); k++) {
          ...
          if (mcp == 0) {
            ...
            streamlog_out(DEBUG6)
                << " collection nb " << i << " nhits " << jN << " hit " << j
                << " contr " << k << std::endl;
            ...
            if (simHit->getNMCContributions() > 0) {
              for (int l = 0; l < simHit->getNMCContributions(); l++) {
                ...
                streamlog_out(DEBUG6)
                    << "     MCPart: " << simHit->getParticleCont(l)
                    << std::endl;
              }
            }
          ...
          streamlog_out(DEBUG3)
              << "      initial true contributor " << k << " id " << mcp->id()
              << " (with E = " << mcp->getEnergy() << " and pdg "
              << mcp->getPDG() << " ) e hit: " << e << std::endl;

          if (remap_as_you_go.find(mcp) != remap_as_you_go.end()) {
            // very first condition: I already know what to do from some earlier
            // hit created by mcp.
            mcp = remap_as_you_go.find(mcp)->second;
            streamlog_out(DEBUG3)
                << "     Hit " << simHit->id() << " / " << caloHit->id()
                << " attributed to " << mcp->id()
                << " because it's origin mcp has already been treated : "
                   "originator case 0 "
                << std::endl;
          } else {
            MCParticle *mother = 0;
            ...
            // Particle gun particles dont have parents, but are "created in the
            // simulation" and have genStat 0
            if (mcp->getGeneratorStatus() == 0 &&
                (mcp->getParents().empty() == false)) {

              // not from generator, find which true particle this
              // hit should really be attributed to, by tracking back the
              // history. (the other case, ie. if the if above is false is "case
              // 1")

              // Two cases to treat:
              //   For some reason, the hit is not attributed to the
              //   incomming particle, but to some particle in the shower.
              //   Just track back to the incomming particle, which might
              //   (usually) be a gen-stat 1 particle from the main vertex. It
              //   can also be from a decay or interaction in the tracking made
              //   by Geant : genstat=0, mother isDecayedInTracker, (or the
              //   production vertex is in the track, but the mother continues
              //   on - ticky case, see comment futher down), or a decyed
              //   particle from the generator (genstat 2) that hits the calo
              //   before decaying (lambdas, K^0_S)

              //   Or: for back-scatterers the calorimiter hit is attributed to
              //   the the last particle *even if this particle both started and
              //   ended in the tracker* !!!! Then we back-track untill we find
              //   a particle which at least started inside the calorimeter, and
              //   then go on backtracking as above. This case is triggered by
              //   the particle linked to the hit being DecayedInTracker, hence
              //   the case where a back-scatter actually ends in the
              //   calorimeter is treated as the first case.
              ...
              streamlog_out(DEBUG2)
                  << "          " << mcp->id() << " gs "
                  << mcp->getGeneratorStatus() << " dint "
                  << mcp->isDecayedInTracker() << " bs " << mcp->isBackscatter()
                  << " ndi " << mcp->vertexIsNotEndpointOfParent() << " npar "
                  << mcp->getParents().size() << " pdg " << mcp->getPDG() << " "
                  << mcp->getVertex()[0] << " " << mcp->getVertex()[1] << " "
                  << mcp->getVertex()[2] << " " << mcp->getEndpoint()[0] << " "
                  << mcp->getEndpoint()[1] << " " << mcp->getEndpoint()[2]
                  << std::endl;
              ...
              while (!this_Kid->isBackscatter() && mother != 0 &&
                     mother->getParents().size() > 0 &&
                     mother->getGeneratorStatus() == 0 &&
                     !mother->isDecayedInTracker()) {
                ...
                if (this_Kid->vertexIsNotEndpointOfParent() !=
                    _invertedNonDestructiveInteractionLogic) {
                  MCParticle *oma =
                      dynamic_cast<MCParticle *>(mother->getParents()[0]);
                  if (oma->isDecayedInTracker()) {
                    streamlog_out(DEBUG1)
                        << "          break out : gandmother " << oma->id()
                        << " gs " << oma->getGeneratorStatus() << " dint "
                        << oma->isDecayedInTracker() << " bs "
                        << oma->isBackscatter() << " ndi "
                        << oma->vertexIsNotEndpointOfParent() << " npar "
                        << oma->getParents().size() << " pdg " << oma->getPDG()
                        << std::endl;
                    break;
                  }
                }
                this_Kid = mother;
                mother = dynamic_cast<MCParticle *>(
                    mother->getParents()[0]); // (assume only one...)

                streamlog_out(DEBUG1)
                    << "          shower-part mother " << mother->id() << " gs "
                    << mother->getGeneratorStatus() << " dint "
                    << mother->isDecayedInTracker() << " bs "
                    << mother->isBackscatter() << " ndi "
                    << mother->vertexIsNotEndpointOfParent() << " npar "
                    << mother->getParents().size() << " pdg "
                    << mother->getPDG() << std::endl;
              }

              // Further treatment (basically determining if it this_Kid or
              // mother that enetered the calorimeter) based on why we left the
              // while-loop

              // here one of the while conditions is false, ie. at least one of
              // " kid is back-scatter", "no mother" , "mother has no parents",
              // "mother is from generator", or "mother did decay in tracker"
              // must be true. We know that this_Kid fulfills all the
              // while-conditions except the first: obvious if at least one
              // iteration of the loop was done, since it was the mother in the
              // previous iteration, but also true even if the loop wasn't
              // transversed, due to the conditions to at all enter this block
              // of code (explicitly must be simulator particle with mother,
              // implicitly must have ended in the calo, since it did make calo
              // hits.)
              if (this_Kid->isBackscatter()) {
                // case 2: Kid is back-scatterer. It has thus started in a calo,
                // and entered
                //  from there into the tracking volume, and did cause hits
                //  after leaving the tracker volume again ->  this_Kid started
                //  before the calo, and is the one
                remap_as_you_go[mcp] = this_Kid;
                mcp = this_Kid;
                streamlog_out(DEBUG3)
                    << "     Hit " << simHit->id() << " / " << caloHit->id()
                    << " attributed to kid " << mcp->id()
                    << " because it's origin is a back-scatter : originator "
                       "case 2 "
                    << std::endl;
                streamlog_out(DEBUG2)
                    << "          " << this_Kid->id() << " gs "
                    << this_Kid->getGeneratorStatus() << " dint "
                    << this_Kid->isDecayedInTracker() << " bs "
                    << this_Kid->isBackscatter() << " npar "
                    << this_Kid->getParents().size() << " pdg "
                    << this_Kid->getPDG() << std::endl;

              } else if (mother->isDecayedInTracker()) { // the clear-cut case:
                remap_as_you_go[mcp] = this_Kid;
                mcp = this_Kid; // this_Kid started before the calo, and is the
                                // one the hit should be attributed to

                streamlog_out(DEBUG3)
                    << "     Hit " << simHit->id() << " / " << caloHit->id()
                    << " attributed to kid " << mcp->id()
                    << " because it's origin is in tracker : originator case 3 "
                    << std::endl;

              } else { // the other three cases, ie. one or sveral of "no
                       // mother", "no grand-mother",
                       //  "generator particle" + that we know that "mother
                       //  decayed in calo"
                if (mother == 0) { // ... which of course implies no
                                   // grand-mother, and no gen stat of the
                                   // mother as well -> should not be possible !

                  streamlog_out(WARNING)
                      << "  MCparticle " << this_Kid->id()
                      << " is a simulation particle, created in the "
                         "calorimeter by nothing . "
                      << std::endl;

                  remap_as_you_go[mcp] = this_Kid;
                  mcp = this_Kid; // can't do better than that.

                } else { // here we know: "mother exists, but decayed in calo".
                         // In addition, two posibilities: mother is generator
                         // particle, or there was no grand-parents. One or both
                         // must be true here. Here it gets complicated, because
                         // what we want to know is whether this_Kid started in
                         // the tracker or not. Unluckily, we don't know that
                         // directly, we only know where the mother ended. IF
                         // ithe mother ended in the tracker, there is no
                         // problem, and has already been treated, but if it
                         // ended in the calo, it is still possible that
                         // this_Kid came from a "non-destructive interaction"
                         // with the tracke-detector material. This we now try
                         // to figure out.

                  if (this_Kid->vertexIsNotEndpointOfParent() ==
                      _invertedNonDestructiveInteractionLogic) {
                    // This bizare condition is due to a bug in LCIO (at least
                    // for the DBD samples.
                    // this_Kid->vertexIsNotEndpointOfParent() should be true in
                    // the "non-destructive interaction", but it isn't: actually
                    // it is "false", but is "true" for the for particles that
                    // *do* originate at the end-vertex of their parent. This is
                    // a bug in Mokka. Hence the above ensures that there was NO
                    // "non-destructive interaction", and it is clear this_Kid
                    // was created at the end-point of the mother. The mother is
                    // either a generator particle (to be saved), or the "Eve"
                    // of the decay-chain (or both). So mother is the one to
                    // save and assign the hits to:
                    remap_as_you_go[mcp] = mother;
                    mcp = mother; // mother started at ip, and reached the calo,
                                  // and is the one the hit should be attributed
                                  // to.

                    streamlog_out(DEBUG3)
                        << "     Hit " << simHit->id() << " / " << caloHit->id()
                        << " attributed to mother " << mcp->id()
                        << " because it is a generator particle or started in "
                           "tracker : originator case 4 "
                        << std::endl;

                  } else { // here we DO have a "non-destructive interaction".
                           // Unluckily, we cant directly know if this took
                           // place in the tracker (in which case we should keep
                           // this_Kid as the mcp to save and assign hits to),
                           // or not. We will play a few clean tricks to find
                           // the cases where it either certain that the
                           // "non-destructive interation" was in the tracker,
                           // or that it was in the calo. This reduces the
                           // number of uncertain cases to play dirty tricks
                           // with.

                    // Clean tricks to play: look at the sisters of this_Kid:
                    // with some luck one of them is a promptly decaying
                    // particle, eg. a pi0.
                    // This sister will be flagged as decayed in calo/tracker,
                    // and from that we know for certain that the
                    // "non-destructive interaction" was in the calo/tracker.
                    // It can also be that one of the sisters is flagged as a
                    // back-scatter, which only happens in the calo. If the
                    // particles grand-mother is decayed in calo, and the mother
                    // isn't from a "non-destructive interaction", the
                    // "non-destructive interaction" was in the calo.

                    // Finally, we can check the distance of the end-point of
                    // the mother (sure to be in the calo) to the vertex of
                    // this_Kid. If this is small, this_Kid *probably* started
                    // in the calo.
                    ...
                    for (unsigned kkk = 0; kkk < mother->getDaughters().size();
                         kkk++) {
                      MCParticle *sister = dynamic_cast<MCParticle *>(
                          mother->getDaughters()[kkk]);
                      if (sister == this_Kid)
                        continue;
                      if (abs(sister->getVertex()[0] -
                              this_Kid->getVertex()[0]) > 0.1 ||
                          abs(sister->getVertex()[1] -
                              this_Kid->getVertex()[1]) > 0.1 ||
                          abs(sister->getVertex()[2] -
                              this_Kid->getVertex()[2]) > 0.1)
                        continue; // must check that it is the same vertex:
                      ...
                      // any pi0:s at all ? (it doesn't matter that we break at
                      // the two cases above, because if we do, it doesn't
                      // matter if there are pi0 sisters or not !)
                      if (sister->getPDG() == 111) {
                        has_pi0 = 1;
                      }
                    }
                    // if not already clear-cut, calculate distance vertext to
                    // mother end-point
                    if (starts_in_tracker != 1 && has_bs != 1 && has_pi0 != 1 &&
                        oma_in_calo != 1) {
                      rdist = sqrt(pow(mother->getEndpoint()[0] -
                                           this_Kid->getVertex()[0],
                                       2) +
                                   pow(mother->getEndpoint()[1] -
                                           this_Kid->getVertex()[1],
                                       2) +
                                   pow(mother->getEndpoint()[2] -
                    ...
                    streamlog_out(DEBUG1)
                        << "          " << starts_in_tracker << " " << has_pi0
                        << " " << has_bs << " " << oma_in_calo << std::endl;
                    //                    }
                    if (starts_in_tracker ==
                        1) { // this_Kid is a clear-cut hit-originator

                      remap_as_you_go[mcp] = this_Kid;
                      mcp =
                          this_Kid; // this_Kid started before the calo, and is
                                    // the one the hit should be attributed to

                      streamlog_out(DEBUG3)
                          << "     Hit " << simHit->id() << " / "
                          << caloHit->id() << " attributed to kid " << mcp->id()
                          << " because it's origin could be deduced to be in "
                             "tracker : originator case 5 "
                          << std::endl;
                      ...
                      streamlog_out(DEBUG2)
                          ...
                          << this_Kid->vertexIsNotEndpointOfParent()
                          << std::endl;
                      streamlog_out(DEBUG1)
                          << " starts in tracker " << std::endl;

                    } else if (has_pi0 != 0 || has_bs != 0 ||
                               oma_in_calo != 0) { // clear-cut case of this_Kid
                                                   // starting in the calo.
                                                   // We do know that the mother
                      // is a generator particle and/or the "Eve" of the
                      // cascade, so we should attribute hits to the mother and
                      // save it.

                      remap_as_you_go[mcp] = mother;
                      mcp = mother; // mother started at ip, and reached the
                                    // calo, and is the one the hit should be
                                    // attributed to.

                      streamlog_out(DEBUG3)
                          << "     Hit " << simHit->id() << " / "
                          << caloHit->id() << " attributed to mother "
                          << mcp->id()
                          << " because it's origin could be deduced to be in "
                             "tracker : originator case 6 "
                          << std::endl;
                      streamlog_out(DEBUG2)
                          << "        Case 6 details: kid starts in calo "
                          << " " << this_Kid->getVertex()[1] << " "
                          << this_Kid->getVertex()[2] << " " << rdist << " "
                          << has_pi0 << " " << has_bs << " " << oma_in_calo
                          << std::endl;

                    } else { // un-clear case: no pi0 nor back-scatteres among
                             // the sisters to help to decide. Use distance
                             // this_Kid-startpoint to mother-endpoint. We know
                             // that the latter is in the calo, so if this is
                             // small, guess that the start point of this_Kid is
                             // also in the calo. Calos are dense, so typically
                             // in the case the "non-destructive interaction" is
                             // in the calo, one would guess  that the distance
                             // is small, ie. large distance -> unlikely that it
                             // was in the calo.

                      if (rdist >
                          200.) { // guess "non-destructive interaction" not in
                                  // calo -> this_Kid is originator

                        remap_as_you_go[mcp] = this_Kid;
                        mcp = this_Kid; // this_Kid started before the calo, and
                                        // is the one the hit should be
                                        // attributed to
                        streamlog_out(DEBUG3)
                            << "     Hit " << simHit->id() << " / "
                            << caloHit->id() << " attributed to kid "
                            << mcp->id()
                            << " because it's origin is guessed to be in "
                               "tracker : originator case 7 "
                            << std::endl;
                        streamlog_out(DEBUG2)
                            << "        Case 7 details: guess kid starts in "
                               "tracker "
                            << " " << this_Kid->getVertex()[1] << " "
                            << this_Kid->getVertex()[2] << " " << rdist << " "
                            << has_pi0 << " " << std::endl;
                        ...
                        streamlog_out(DEBUG3)
                            ...
                            << caloHit->id() << " attributed to mother "
                            << mcp->id()
                            << " because it's origin is guessed in tracker : "
                               "originator case 8 "
                            << std::endl;
                        streamlog_out(DEBUG2)
                            << "        Case 8 details:  guess kid starts in "
                               "calo "
                            << " " << this_Kid->getVertex()[1] << " "
                            << this_Kid->getVertex()[2] << " " << rdist << " "
                            << has_pi0 << " " << std::endl;
              ...
              // first case: the hit generating mcp itself already fulfills the
              // firest criterium, ie. it is a generator particle
              streamlog_out(DEBUG3)
                  << "     Hit " << simHit->id() << " / " << caloHit->id()
                  << " attributed to " << mcp->id()
                  << " because it's origin is a generator particle : "
                     "originator case 1 "
                  << std::endl;
          ...
          streamlog_out(DEBUG4)
              << "    Final assignment for contribution " << k << " to "
              << simHit->id() << " / " << caloHit->id() << " : " << mcp->id()
              << std::endl;
          streamlog_out(DEBUG4)
              << "      gs " << mcp->getGeneratorStatus() << " dint "
              << mcp->isDecayedInTracker() << " bs " << mcp->isBackscatter()
              << " npar " << mcp->getParents().size() << " pdg "
              << mcp->getPDG() << " " << mcp->getEndpoint()[0] << " "
              << mcp->getEndpoint()[1] << " " << mcp->getEndpoint()[2]
              << std::endl;
          ...
          for (size_t jk = 0; jk < caloHits.size(); jk++) {
            chitTruthRelNav.addRelation(
                caloHits[jk], mcp,
                e * ((CalorimeterHit *)caloHits[jk])->getEnergy() / allRecoEn);
          }
  ...
  streamlog_out(DEBUG6)
      << " *** Sorting out simHit<->MCParticle connections : DONE "
      << std::endl;
  streamlog_out(DEBUG6)
      << " *** Sorting out Cluster<->MCParticle using simHit<->MCParticle, "
         "re-assigning the latter in some rare cases."
      << std::endl;
  ...
  streamlog_out(DEBUG6) << std::endl;
  streamlog_out(DEBUG6) << " ================= " << std::endl;
  streamlog_out(DEBUG6) << " Treating clusters. There are " << nCluster
                        << " of them " << std::endl;
  ...
  for (int i = 0; i < nCluster; i++) {
    ...
    // We need to find all seen hits this clutser is made of, which sim hits
    // each of the seen hits came from, and finally which true particles
    // actually created each sim hit. Contrary to the sim tracker hits above, a
    // sim-calo hit can be made by several true particles. They also have a
    // signal size (energy) value. In addition, the true particle creating
    // sometimes needs to be back-tracked to the particle actually entering tha
    // calorimeter.
    ...
    streamlog_out(DEBUG5) << "  ================= " << std::endl;
    streamlog_out(DEBUG5) << "Cluster clu = " << clu->id() << " (i = " << i
                          << " ) with " << cluHits.size() << " hits "
                          << std::endl;
    ...
    for (CalorimeterHitVec::const_iterator hitIt = cluHits.begin();
      ...
      streamlog_out(DEBUG4) << std::endl;
      streamlog_out(DEBUG4)
          << "     Treating hit = " << hit->id() << " e " << hit->getEnergy()
          << " nb sim hits : " << simHits.size() << std::endl;
      ...
      for (LCObjectVec::const_iterator objIt = simHits.begin();
        ...
        SimCalorimeterHit *simHit =
            dynamic_cast<SimCalorimeterHit *>(*objIt); // ... and a sim hit ....

        double calib_factor =
            hit->getEnergy() /
            simHit->getEnergy(); // In this place, this defn of calib_factor is
                                 // still appropriate for split hits (DJ)

        streamlog_out(DEBUG3)
            << "     simhit = " << simHit->id() << " has "
            << simHit->getNMCContributions() << " contributors " << std::endl;
        ...
        for (int j = 0; j < simHit->getNMCContributions(); j++) {
          ...
          streamlog_out(DEBUG3)
              << "     true contributor = " << mcp->id() << " e: " << e
              << " mapped to " << remap_as_you_go.find(mcp)->second->id()
              << std::endl;
          if (mcp == 0) {
            streamlog_out(DEBUG7) << "      simhit = " << simHit
                                  << " has no creator " << std::endl;
            // streamlog_out( DEBUG7 ) <<"         true contributor = "<< mcp <<
            // " e: " << e
            //    <<" mapped to " <<remap_as_you_go.find(mcp)->second <<
            //    std::endl;
            continue;
          }
          mcp = remap_as_you_go.find(mcp)->second;
          mcpEnergy[mcp] +=
              e;     // count the hit-energy caused by this true particle
          eTot += e; // total energy
          ehit += e;
        } // mc-contributon-to-simHit loop
      }   // simHit loop

      streamlog_out(DEBUG4) << "     summed contributed e: " << ehit
      ...
      if (nsimhit == 0) {
        ...
        streamlog_out(DEBUG5)
            ...
            << " " << clu->getPosition()[0] << " " << clu->getPosition()[1]
            << " " << clu->getPosition()[2] << " " << std::endl;
      }
    } // hit loop

    if (no_sim_hit == 1) {
      streamlog_out(DEBUG6)
          << "   Warning, there are  sim-less calohits in cluster " << clu->id()
          << std::endl;
    }
    streamlog_out(DEBUG5) << std::endl;
    streamlog_out(DEBUG5) << "    Sum of calohit E: " << ecalohitsum
                          << " cluster E " << clu->getEnergy()
                          << " Sum of Simcalohit E: " << eTot
                          << "  no_sim_hit: " << no_sim_hit
                          << ". Energy from unknow source : "
                          << ecalohitsum_unknown << std::endl;
    if (eTot ==
        0.0) { // fixme - this might happen if clusters are from Lcal/Muon only
    ...
    // At this point, we have a list of all true particles contributiong to this
    // cluster. We now need to sum up the energies each particle contributes
    // with and do further parsing.
    ...
    MCParticleVec
        moreMCPs; // vector that will contain true particles contributing, that
    ...
    streamlog_out(WARNING) << "CHECKPOINT1" << std::endl;

    for (MCPMapDouble::iterator it =
             mcpEnergy.begin(); // iterate trough the map.
         it != mcpEnergy.end(); ++it) {

      //streamlog_out(WARNING) << "it->first : " << it->first << std::endl;
      
      //MODIFIED HERE!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      if (it->first == 0 //true
      ) { // ( if == 0, this cluster contains some (but not all) sim-hits
               // with unknown origin.
               //   If *all* sim-hits would have had unknown origin, we would
               //   already have "continue":ed above)
      ...
      streamlog_out(WARNING) << "CHECKPOINT3" << std::endl;

      if (it->first->getGeneratorStatus() ==
          1) { // genstat 1 particle, ie. it is a bona fide
               // creating true particle: enter it into
               // the list, and note how much energy it
               // contributed.
        streamlog_out(WARNING) << "CHECKPOINT4a" << std::endl;
        theMCPs.push_back(it->first);
        MCPes.push_back(it->second);
        ifound++;
      } else { // not genstat 1. What should we do with it ?
        if (it->first->getParents().size() != 0) {
          streamlog_out(WARNING) << "it->first->getParents().size() : "
                                 << it->first->getParents().size() << std::endl;

          mother = dynamic_cast<MCParticle *>(it->first->getParents()[0]);
          streamlog_out(WARNING) << "CHECKPOINT5" << std::endl;
        } else {
          mother = 0;
        }
        streamlog_out(WARNING) << "CHECKPOINT6" << std::endl;
        if (mother != 0) {
          if (mother->isDecayedInTracker() && // ... so the partic apeared in
                                              // the tracker ...
            ...
            MCPes.push_back(it->second);
            ifound++;
          } else { // else: if the mother is a BS, add to the  moreMCPs-list,
            ...
            streamlog_out(DEBUG2) << " case 1 for " << it->first->id()
                                  ...
                                  << mother->getGeneratorStatus() << " dint "
                                  << mother->isDecayedInTracker() << " bs "
                                  << mother->isBackscatter() << " pdg "
                                  << mother->getPDG() << std::endl;
            if (mother->isBackscatter() == 1) {
              moreMCPs.push_back(it->first);
              moreMCPes.push_back(it->second);
              morefound++;
            } else {
              theMCPs.push_back(it->first);
              MCPes.push_back(it->second);
          ...
          streamlog_out(DEBUG6)
              << " case 2 for " << it->first->id() << "(morefound=" << morefound
              << ")" << std::endl;
          moreMCPs.push_back(it->first);
          moreMCPes.push_back(it->second);
          morefound++;
    ...
    // We now have two lists of contributing true particles, one containing
    // those that will be put to the skimmed list in any case, one with those
    // that will be put there only is they produced a visible signal. In
    // addition, we have the total contribution to the shower from each true
    // particle in separate lists.

    if (morefound > 0) {
      for (int iii = 0; iii < morefound; iii++) {
        streamlog_out(DEBUG2)
            << " iii, moreMCPes[iii], moreMCPs[iii], gs " << iii << " "
            << moreMCPes[iii] << " " << moreMCPs[iii]->id() << " "
            << moreMCPs[iii]->getGeneratorStatus() << std::endl;
      }
      for (int iii = 0; iii < ifound; iii++) {
        streamlog_out(DEBUG2)
            << " iii, MCPes[iii], theMCPs[iii] " << iii << " " << MCPes[iii]
            << " " << theMCPs[iii]->id() << std::endl;
      }
      streamlog_out(DEBUG3) << "   morefond: " << morefound << std::endl;
    }

    // figure out what to do with the cases where true particles not
    // automatically in the skimmed list contributed to the cluster: Normally,
    // the back-tracking above has done the job, but sometimes one has a neutron
    // or a photon playing pin-ball between the calorimeter and the tracker, so
    // that a back-scatter scatters back from the tracker into the same cluster
    // it came from. In that case, we should ignore the back scatter (normally,
    // of course, a back-scatterer is - if it reaches a calorimeter - in a
    // different cluster and should be kept as an originator). I couldn't figure
    // out a more efficient way of figuring this out than the
    ...
    for (int iii = 0; iii < morefound; iii++) {
      if (moreMCPs[iii]->getParents().size() != 0) {
        ...
        streamlog_out(DEBUG2)
            << "   iii: " << iii << " mother: " << mother->id() << std::endl;
      } else {
        mother = 0;
      }
      streamlog_out(DEBUG2)
      ...
      streamlog_out(DEBUG2)
          ...
          << " dint " << moreMCPs[iii]->isDecayedInTracker() << " bs "
          << moreMCPs[iii]->isBackscatter() << " npar "
          << moreMCPs[iii]->getParents().size() << " pdg "
          << moreMCPs[iii]->getPDG() << " dinc "
          << moreMCPs[iii]->isDecayedInCalorimeter() << " bye "
          << moreMCPs[iii]->hasLeftDetector() << " stop "
          << moreMCPs[iii]->isStopped() << std::endl;

      while (mother != 0 &&
             mother->getGeneratorStatus() != 2) { // back-track to the
        ...
        streamlog_out(DEBUG2)
            ...
            << mother->getGeneratorStatus() << " dint "
            << mother->isDecayedInTracker() << " "
            << " bs " << mother->isBackscatter() << " pdg " << mother->getPDG()
            << " dinc " << mother->isDecayedInCalorimeter() << std::endl;
        streamlog_out(DEBUG2)
            << "       mother vert: " << mother->getVertex()[0] << " "
            << mother->getVertex()[1] << " " << mother->getVertex()[2]
            << std::endl;
        streamlog_out(DEBUG2)
        ...
        // find out if this ancestor is a back-scatterer actually directly
        // giving rise to hits in this cluster. If so, we attribute the hits of
        // moreMCPs[iii] to that true particle instead.
        ...
        for (int kkk = 0; kkk < ifound; kkk++) {
          ...
          if (tmcp == mother) {
            ...
            streamlog_out(DEBUG3)
                << "        found " << moreMCPs[iii] << "(iii= " << iii << ")"
                << kkk << " to be related to " << mother->id()
                << " add e : " << moreMCPes[iii] << std::endl;
            LCObjectVec hitvec =
                chitTruthRelNav.getRelatedFromObjects(moreMCPs[iii]);
            FloatVec evec =
                chitTruthRelNav.getRelatedFromWeights(moreMCPs[iii]);
            int one_mod_done = 0;
            ...
            if (one_mod_done == 1) {
              ...
              streamlog_out(DEBUG3) << "        However, no hits related to "
                                       "the current cluster found ??"
                                    << std::endl;
            }
          }
        }
    ...
    endwhile:
      if (mother == 0 || mother->getGeneratorStatus() == 2) {
        ...
        theMCPs.push_back(moreMCPs[iii]);
        MCPes.push_back(moreMCPes[iii]);
        ifound++;
      }
    }
    streamlog_out(DEBUG5) << " cluster " << clu->id()
                          << " , E = " << clu->getEnergy()
    ...
    for (int iii = 0; iii < ifound; iii++) {
      float weight = (MCPes[iii] / eTot) *
                     (clu->getEnergy() - ecalohitsum_unknown) /
                     clu->getEnergy();
      ...
      if (theMCPs[iii] == 0) {
        ...
        streamlog_out(ERROR)
            << " cluster " << clu->id() << " has " << MCPes[iii] << " GeV of "
            << eTot << " GeV [ " << weight << " ] true energy "
      ...
      streamlog_out(DEBUG5)
          << " cluster " << clu->id() << " has " << MCPes[iii] << " of " << eTot
          << "  [ " << weight << " ] "
      ...
      if (theMCPs[iii]->getGeneratorStatus() == 0 &&
          (theMCPs[iii]->getParents().empty() == false)) {
        streamlog_out(DEBUG5)
  ...
  streamlog_out(DEBUG6) << " *** Sorting out Cluster<->MCParticle : DONE"
                        << std::endl;
  streamlog_out(DEBUG6) << " *** Guessing Cluster<->MCParticle in cases where "
                           "MCParticle<->simHit is broken (LCAL/DBD)"
                        << std::endl;
  ...
  // (This is from the original MyRecoMCTruthLinker. I didn't revise it/MB)
  ...
  for (unsigned i = 0; i < missingMC.size(); ++i) {
    ...
    if (maxProd > 0.) {
      ...
      streamlog_out(DEBUG5)
          << "  neutral cluster particle recovered" << clu->getEnergy()
          << " maxProd: " << maxProd << " px: " << closestMCP->getMomentum()[0]
          << " py: " << closestMCP->getMomentum()[1]
          << " pz: " << closestMCP->getMomentum()[2] << std::endl;
      ...
      //  CalorimeterHit* hit = * hitIt ;  // ... a calo seen hit ...
      //  streamlog_out( DEBUG5 )
      //  <<"recuperated   hit = "<< hit << " e " << hit->getEnergy()<<
      //  std::endl;
  ...
  streamlog_out(DEBUG6) << " *** Cluster linking complete, create collection "
                        << std::endl;
...
void MyRecoMCTruthLinker::particleLinker(
    LCCollection *mcpCol, LCCollection *particleCol, LCCollection *ttrlcol,
    LCCollection *ctrlcol, LCCollection *trtlcol, LCCollection *trclcol,
    LCCollection **ptrlcol, LCCollection **trplcol) {

  LCRelationNavigator particleTruthRelNav(LCIO::RECONSTRUCTEDPARTICLE,
                                          LCIO::MCPARTICLE);
  LCRelationNavigator truthParticleRelNav(LCIO::MCPARTICLE,
  ...
  for (int i = 0; i < nPart; ++i) {
    ...
    ReconstructedParticle *part =
        dynamic_cast<ReconstructedParticle *>(particleCol->getElementAt(i));
    ...
    streamlog_out(DEBUG3) << "      Treating particle " << part->id()
                          << " with index " << i << " it has " << ntrk
                          << " tracks, and " << nclu << " clusters "
                          << std::endl;
    ...
    if (ntrk > 1) {
      ...
      for (int j = 0; j < ntrk; j++) {
        nhit[j] = 0;
        for (unsigned kkk = 0;
             kkk < tracks[j]->getSubdetectorHitNumbers().size(); kkk++) {
          nhit[j] += tracks[j]->getSubdetectorHitNumbers()[kkk];
          nhitT += tracks[j]->getSubdetectorHitNumbers()[kkk];
        }
        streamlog_out(DEBUG2)
            << "         Track " << tracks[j]->id() << " with index " << j
            << " has " << nhit[j] << " hits " << std::endl;
      }
      streamlog_out(DEBUG2)
          << "         Total : " << nhitT << " hits " << std::endl;
    } else {
      nhit[0] = 1;
      nhitT = 1;
    }
          ...
          if (mcvec[0] != 0) {
            ...
            for (int kkk = 0; kkk < ntp; kkk++) {
              if (!_FullRecoRelation &&
                  www[kkk] * (float(nhit[j]) / float(nhitT)) > maxwgt) {
    ...
    if (_FullRecoRelation || ntrk == 0) {
      ...
      if (nclu > 1) {
        ...
        for (int j = 0; j < nclu; j++) {
          ...
          streamlog_out(DEBUG2)
              << "        Cluster " << clusters[j]->id() << " with index " << j
    ...
    if (_FullRecoRelation) {
      for (std::map<MCParticle *, int>::iterator mcit = mcmap.begin();
        ...
        streamlog_out(DEBUG4)
            << "     particle " << part->id() << " has weight " << mcit->second
            ...
            << " id: " << mcit->first->id() << std::endl;

        particleTruthRelNav.addRelation(part, mcit->first, mcit->second);
      }
    } else {
      if (mcmax != NULL) {
        // AS: FixMe: There is still something going wrong with particles from
        // the particle gun. Although there are perfect PFOs no link with the
        // MCParticle is established.
        particleTruthRelNav.addRelation(part, mcmax, maxwgt);
        streamlog_out(DEBUG3)
            << "      particle " << part->id() << " has weight " << maxwgt
        ...
        streamlog_out(WARNING)
            ...
            << ". Particle charge and ntracks : " << part->getCharge() << " "
            << ntrk << " but no mcparticle found " << std::endl;
      }
    }
  }
  // The reverse map. Differs in what the weight means: here it means:
  ...
  for (int i = 0; i < nMCP; i++) {
    ...
    for (unsigned j = 0; j < partvec.size(); j++) {
      ReconstructedParticle *msp =
          dynamic_cast<ReconstructedParticle *>(partvec[j]);
      ...
      streamlog_out(DEBUG4)
          << "    True Particle " << mcp->id() << " ( pdg " << mcp->getPDG()
          << " ) has weight " << c_wgt << " / " << t_wgt << "( " << int(wgt)
          << " ) to particle " << msp->id() << "  with " << cluvec_p.size()
          << " clusters, and " << trkvec_p.size() << " tracks " << std::endl;
    }
  }
  streamlog_out(DEBUG6) << " particle linking complete, create collection "
                        << std::endl;
  *ptrlcol = particleTruthRelNav.createLCCollection();
  *trplcol = truthParticleRelNav.createLCCollection();
}
void MyRecoMCTruthLinker::makeSkim(LCCollection *mcpCol, LCCollection *ttrlcol,
                                   LCCollection *ctrlcol,
  ...
  for (int i = 0; i < nMCP; i++) {
    ...
    if (!mcp->isCreatedInSimulation() || mcp->getGeneratorStatus() != 0) {
    ...
    } else { // of those created in the simulation we keep those that actually
             // are reconstructed including all parents

      //  truthRelNav is the one we created above, remember, so here we make
      //  sure that all true particles related to seen ones (with the logic we
      //  used there) really will be in the skimmed collection !

      const LCObjectVec &trackObjects =
          trackTruthRelNav.getRelatedFromObjects((LCObject *)mcp);
      const LCObjectVec &clusterObjects =
          clusterTruthRelNav.getRelatedFromObjects((LCObject *)mcp);
      ...
      if (trackObjects.size() > 0 || clusterObjects.size() > 0) {
        ...
        streamlog_out(DEBUG5)
            ...
            << " pz: " << mcp->getMomentum()[2] << " mass: " << mcp->getMass()
            << " pdg " << mcp->getPDG() << std::endl;

        // keepMCParticles also flags all parents of a kept particle,
        // guaranteeing that the history of any contributor to a detected signal
        // will be kept !
    ...
    // --- loop again and add daughters of particles that are in the skimmed
    // list and have a pdg in
    //     the parameter vector 'KeepDaughtersPDG

  streamlog_out(DEBUG5) << " First loop done. Now search for KeepDaughtersPDG:s"
                        << std::endl;
  ...
  if (_saveBremsstrahlungPhotons) {
    for (int i = 0; i < nMCP; i++) {
      ...
      if ((abs(mcp->getPDG()) == 22) &&
          (mcp->getEnergy() > _bremsstrahlungEnergyCut)) {
  ...
  for (int i = 0; i < nMCP; i++) {
    ...
    // keep the daughters of all decays in flight of particles in the pdg list
    // (default: gamma, pi0, K0s)
    if (mcp->ext<MCPKeep>() == true &&
        mcp->isDecayedInTracker()) { //&& !mcp->isStopped()  ){
      ...
      if (_pdgSet.find(thePDG) != _pdgSet.end()) {
        ...
        //                                mcp->isCreatedInSimulation() <<
        //                                std::endl
        //                                << " isBackscatter :" <<
        //                                mcp->isBackscatter() << std::endl
        //                                << " vertexIsNotEndpointOfParent :" <<
        //                                mcp->vertexIsNotEndpointOfParent() <<
        //                                std::endl
        //                                << " isDecayedInTracker :" <<
        //                                mcp->isDecayedInTracker()       <<
        //                                std::endl
        //                                << " isDecayedInCalorimeter :" <<
        //                                mcp->isDecayedInCalorimeter()       <<
        //                                std::endl
        //                                << " hasLeftDetector :" <<
        //                                mcp->hasLeftDetector()     <<
        //                                std::endl
        ...
        streamlog_message(DEBUG5, if (mcp->getParents().size()),
...
void MyRecoMCTruthLinker::keepMCParticle(MCParticle *mcp) {

  mcp->ext<MCPKeep>() = true;

  const MCParticleVec &parents = mcp->getParents();

  streamlog_out(DEBUG3) << " keepMCParticle keep particle with pdg : "
                        << mcp->getPDG() << std::endl;
...
void MyRecoMCTruthLinker::check(LCEvent *evt) {
  ...
  if (isFirstEvent()) {
    ...
    hmcp_e = AIDAProcessor::histogramFactory(this)->createCloud1D(
        "hmcp_e", " energy/GeV - all ", 100);
    //       createHistogram1D( "hmcp_e", " energy/GeV ", 100, 0. , 10. ) ;

    hmcp_etot = AIDAProcessor::histogramFactory(this)->createCloud1D(
        "hmcp_etot", " total energy/GeV ", 100);
    //       createHistogram1D( "hmcp_etot_e", " energy/GeV ", 1000, 0. , 1000.
    //       ) ;

    hmcp_n = AIDAProcessor::histogramFactory(this)->createCloud1D(
        "hmcp_n", " # generated stable particles ", 100);

    hmcp_ntot = AIDAProcessor::histogramFactory(this)->createCloud1D(
        "hmcp_ntot", "  total # particles ", 100);

    hmcpsk_e = AIDAProcessor::histogramFactory(this)->createCloud1D(
        "hmcpsk_e", " energy/GeV - all ", 100);
    //       createHistogram1D( "hmcpsk_e", " energy/GeV ", 100, 0. , 10. ) ;

    hmcpsk_etot = AIDAProcessor::histogramFactory(this)->createCloud1D(
        "hmcpsk_etot", " total energy/GeV ", 100);
    //       createHistogram1D( "hmcpsk_etot_e", " energy/GeV ", 1000, 0. ,
    //       1000. ) ;

    hmcpsk_n = AIDAProcessor::histogramFactory(this)->createCloud1D(
        "hmcpsk_n", " # generated stable particles ", 100);

    hmcpsk_ntot = AIDAProcessor::histogramFactory(this)->createCloud1D(
        "hmcpsk_ntot", "  total # particles ", 100);
  }
  ...
  try {
    ...
    streamlog_out(DEBUG9) << "RecoMCTructh::Check(): MCParticle collection \""
  ...
  try {
    ...
    streamlog_out(DEBUG9)
        ...
        << _mcParticlesSkimmedName << "\" does not exist, skipping"
        << std::endl;
  }
...
const LCObjectVec *MyRecoMCTruthLinker::getSimHits(TrackerHit *trkhit,
                                                   const FloatVec *weights) {

  const LCObjectVec *obj =
      &_navMergedTrackerHitRel->getRelatedToObjects(trkhit);

  if (obj->empty() == false) {

    if (weights != 0)
      weights = &_navMergedTrackerHitRel->getRelatedToWeights(trkhit);

  } else {
    streamlog_out(WARNING) << "getSimHits :  TrackerHit : " << trkhit
...
void MyRecoMCTruthLinker::end() {

  streamlog_out(DEBUG6) << " processed " << _nEvt << " events in " << _nRun
                        << " runs " << std::endl;
}

/** helper function to get collection safely */
inline lcio::LCCollection *getCollection(lcio::LCEvent *evt,
...
void MyRecoMCTruthLinker::mergeTrackerHitRelations(LCEvent *evt) {
  ...
  for (unsigned i = 0; i < nCol; ++i) {
    ...
    if (col != 0) {
      ...
      streamlog_out(DEBUG2)
          << " mergeTrackerHitRelations: input collection missing : "
          << _colNamesTrackerHitRelations[i] << std::endl;
    }
  }

  streamlog_out(DEBUG2)
      << " mergeTrackerHitRelations: copied collection parameters ... "
      << std::endl;
...
void MyRecoMCTruthLinker::mergeCaloHitRelations(LCEvent *evt) {
  ...
  streamlog_out(DEBUG2)
      << " mergeCaloHitRelations: copied collection parameters ... "
      << std::endl;
  ...
  if (nCol != 0)
    _navMergedCaloHitRel = new LCRelationNavigator(_mergedCaloHitRelCol);
}

const LCObjectVec *MyRecoMCTruthLinker::getCaloHits(CalorimeterHit *calohit,
                                                    const FloatVec *weights) {
  ...
  if (obj->empty() == false) {
    ...
    if (weights != 0)
      weights = &_navMergedCaloHitRel->getRelatedToWeights(calohit);

  } else {

    streamlog_out(DEBUG5) << "getCaloHits :  CalorimeterHit : " << calohit
                          << " has no sim hits related. CellID0 = "
                          << calohit->getCellID0()
...
void MyRecoMCTruthLinker::linkPrinter(LCCollection *mcpCol,
  ...
  for (int i = 0; i < nPart; ++i) {
    ...
    int ntp = mcvec.size();
    streamlog_out(DEBUG5) << "    Particle " << part->id()
                          << " (q: " << int(part->getCharge()) << " ) has "
                          << ntp << " true particles. E= " << part->getEnergy()
                          << " " << clu_e;
    streamlog_out(DEBUG3)
    ...
    if (ntp > 0) {
      if (mcvec[0] != 0) {
        ...
        for (int kkk = 0; kkk < ntp; kkk++) {
          ...
          streamlog_out(DEBUG3)
              << "       " << kkk << " " << mcp->id() << " " << mcp->getPDG()
              << " " << mcp->getEnergy() << std::endl;
          if (part->getCharge() != 0) {
            if (mcp->getCharge() != 0 &&
                (int(www[kkk]) % 10000) / 1000.0 > 0.6) {
              true_charged_E = mcp->getEnergy();
              double ecalo_from_this_true =
                  ((int(www[kkk]) / 10000) / 1000.0) * clu_e;
              if (ecalo_from_this_true != 0.0) {
                ...
                for (unsigned jjj = 0; jjj < partvec.size(); jjj++) {
                  if (partvec[jjj] == part) {
                    total_ecalo_in_this_true =
                        ecalo_from_this_true /
                        (int(www_other_way[jjj] / 10000) / 1000.0);
                    break;
                  }
                }
              }
            } else {
              total_ecalo_neutral_in_charged +=
                  ((int(www[kkk]) / 10000) / 1000.0) * clu_e;
            }
          } else {
            if (mcp->getCharge() == 0) {
              total_e_from_neutrals +=
                  ((int(www[kkk]) / 10000) / 1000.0) * clu_e;
            }
          }
        }
        if (part->getCharge() != 0) {
          streamlog_out(DEBUG5)
              ...
              << " track weight " << total_trk_weight << " cluster weight "
              << total_clu_weight << std::endl;
        } else {
          streamlog_out(DEBUG5)
  ...
  for (int i = 0; i < nMCPart; ++i) {
    ...
    if (nsp > 0) {
      streamlog_out(DEBUG5)
          ...
          << " (q: " << int(mcpart->getCharge()) << " ) has " << nsp
          << " seen particles. E= " << mcpart->getEnergy() << " pt= "
          << sqrt(mcpart->getMomentum()[0] * mcpart->getMomentum()[0] +
                  mcpart->getMomentum()[1] * mcpart->getMomentum()[1]);
      streamlog_out(DEBUG3)
      ...
      if (partvec[0] != 0) {
        ...
        for (int kkk = 0; kkk < nsp; kkk++) {
          total_trk_weight += (int(www[kkk]) % 10000) / 1000.0;
          total_clu_weight += (int(www[kkk]) / 10000) / 1000.0;
          ReconstructedParticle *recopart =
              dynamic_cast<ReconstructedParticle *>(partvec[kkk]);
          streamlog_out(DEBUG3) << "       " << kkk << " " << recopart->id()
