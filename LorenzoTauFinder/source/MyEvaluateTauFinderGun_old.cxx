#include "MyTauFinder/MyEvaluateTauFinderGun.h"
...
MyEvaluateTauFinderGun aEvaluateTauFinderGun;

struct TAU { // Declare  struct type

  float E, px, py, pz, pt, phi, eta, D0, Evis, pxvis, pyvis, pzvis, ptvis,
      phivis, etavis, minv, Eseed;
  int charge, NQ, npfos, decaymode, missed, matched, fake, decayEl,
      decayMu; // Declare member types
  std::vector<int> pfosPdg;
  std::vector<float> pfosDeltaR;
  std::vector<float> pfosPt;
};

bool MyAngleSort(TAU p1, TAU p2) { return fabs(p1.phi) > fabs(p2.phi); }

MyEvaluateTauFinderGun::MyEvaluateTauFinderGun()
    : Processor("MyEvaluateTauFinderGun") {
  // modify processor description
  _description = "MyEvaluateTauFinderGun checks performance of TauFinder and "
...
void MyEvaluateTauFinderGun::init() {
  ...
  tree->Branch("pfosPdg", pfosPdg, "pfosPdg[nTausRec][recNPfos]/I");
  tree->Branch("pfosDeltaR", pfosDeltaR, "pfosDeltaR[nTausRec][recNPfos]/F");
  tree->Branch("pfosPt", pfosPt, "pfosPt[nTausRec][recNPfos]/F");
...
void MyEvaluateTauFinderGun::processRunHeader(LCRunHeader *) { _nRun++; }

void MyEvaluateTauFinderGun::processEvent(LCEvent *evt) {
    ...
    for (int k = 0; k < nT; k++) {
      ...
      */
      ...
      for (unsigned int o = 0; o < tauvec.size(); o++) {
        ...
        pfos_px = tauvec[o]->getMomentum()[0];
        pfos_py = tauvec[o]->getMomentum()[1];
        pfos_pz = tauvec[o]->getMomentum()[2];
        pfos_pt = std::sqrt(pfos_px * pfos_px + pfos_py * pfos_py);
        pfos_eta = std::atanh(pfos_pz /
                              std::sqrt(pfos_px * pfos_px + pfos_py * pfos_py +
                                        pfos_pz * pfos_pz));
        pfos_phi = TMath::ATan2(pfos_py, pfos_px);
        pfos_deltaR = std::sqrt((pfos_phi - phi) * (pfos_phi - phi) +
                                (pfos_eta - eta) * (pfos_eta - eta));

        (rtau.pfosPdg).push_back(tauvec[o]->getType());
        (rtau.pfosPt).push_back(pfos_pt);
        (rtau.pfosDeltaR).push_back(pfos_deltaR);
      ...
      rtau.eta = eta;
      rtau.D0 = D0;
      rtau.npfos = tauvec.size();
      rtau.NQ = NQ;
      rtau.charge = tau->getCharge();
      rtau.Eseed = Eseed;
      rtau.decayEl = decayEl;
      rtau.decayMu = decayMu;
      ...
      std::cout << "rtau.pfosPdg.size(): " << rtau.pfosPdg.size() << '\n';
      rectauvec.push_back(rtau);
      std::cout << "(rectauvec[rectauvec.size()-1].pfosPdg).size(): "
                << (rectauvec[rectauvec.size()-1].pfosPdg).size() << '\n';
      std::cout << "(rectauvec[rectauvec.size()-1].pfosDeltaR).size(): "
                << (rectauvec[rectauvec.size()-1].pfosDeltaR).size() << '\n';
    }
  ...
  for (unsigned int p = 0; p < rectauvec.size(); p++) {
    ...
    recNPfos[p] = rectauvec[p].npfos;
    recNQTracks[p] = rectauvec[p].NQ;

    charge[p] = rectauvec[p].charge;
    matched[p] = rectauvec[p].matched;
    fake[p] = rectauvec[p].fake;
    decayEl[p] = rectauvec[p].decayEl;
    decayMu[p] = rectauvec[p].decayMu;

    // std::cout << "rectauvec[p].pfosPdg.size(): " <<
    // (rectauvec[0].pfosPdg).size() << '\n';

    /*
    std::cout << "rectauvec[p].npfos : " << rectauvec[p].npfos << '\n';

    for (int i = 0; i != rectauvec[p].npfos; ++i) {
      pfosPdg[p][i] = rectauvec[p].pfosPdg[i];
      pfosPt[p][i] = rectauvec[p].pfosPt[i];
      pfosDeltaR[p][i] = rectauvec[p].pfosDeltaR[i];
    }
    */
  }
...
void MyEvaluateTauFinderGun::LoopDaughters(MCParticle *particle, double &Evis,
  ...
  for (unsigned int d = 0; d < particle->getDaughters().size(); d++) {
    ...
    if (daughter->hasLeftDetector() == false ||
      ...
      if (daughter->getGeneratorStatus() == 0) {
...
void MyEvaluateTauFinderGun::LoopDaughtersRelation(
...
int MyEvaluateTauFinderGun::FindDecayMode(MCParticle *tau) {
  /*
   *Decay modes for tau-:
...
void MyEvaluateTauFinderGun::check(LCEvent *) {
  // nothing to check here - could be used to fill checkplots in reconstruction
  // processor
}

void MyEvaluateTauFinderGun::end() {

  streamlog_out(DEBUG) << "MyEvaluateTauFinderGun::end()  " << name()
